...
<head>
	...
	<div id="cabecera">
        ...
        $string='';
        $string=urldecode($cadena);
				if (isset($h)){
						foreach ($h->result() as $row) {
							if($row->$string!=NULL){
							?>
								<a href="<?php echo base_url(); ?>index.php/Producto/cargar/<?php echo($row->$string); ?>">
								<img name="imagen" style="margin:1%;width:313px;height:350px;" alt="<?php echo($row->$string); ?>
								" src="<?php echo base_url(); ?>img/<?php echo($row->$string); ?>.jpg"></a>
							<?php
						}
						}
				}
